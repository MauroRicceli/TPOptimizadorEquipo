...
public class Registro extends JLayeredPane {
	...
	//	txtRoles.setText("( Programador, tester, arquitecto, lider de proyecto )");
	...
	//	txtCalificacion.setText("El valor debe estar en el intervalo [1,5]");
	...
	protected Empleado datosIngresados() {
		...
		if (!roles.contains(rol.toLowerCase())) {
			JOptionPane.showMessageDialog(null, "Error: el rol ingresado no es válido");
			return null;
		}
		return new Empleado(nombre,Integer.parseInt(calificacion),rol.toLowerCase());
	}
